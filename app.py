...
st.title("Import File Builder V2")
st.caption("Convert Underwriting Engine / BETTY output into the blind-offer import layout.")
...
def find_col(df, candidates):
    ...
    def norm(x):
        return str(x).strip().lower().replace("__dup1", "")
...
def build_import(df, offer_accept_by="", close_date="", exclude_no_offer=True):
    ...
    owner_full = get_series(df, ["BETTY_UPDATED_OWNER_FULL_NAME", "Owner Full Name", "Owner Name(s)", "Owner Name", "Owner"], "")
    ...
    out["Address"] = get_series(df, ["BETTY_UPDATED_ADDRESS_LINE1", "Address", "Mail Full Address", "Mail Address"], "")
    out["City"] = get_series(df, ["BETTY_UPDATED_ADDRESS_CITY", "City", "Mail City"], "")
    out["State"] = get_series(df, ["BETTY_UPDATED_ADDRESS_STATE", "State", "Mail State"], "")
    out["Zip"] = get_series(df, ["BETTY_UPDATED_ADDRESS_ZIP", "Zip", "Mail Zip"], "")
    ...
    out["FIPS"] = get_series(df, ["Parcel FIPS", "FIPS"], "")
